...
public:
    Load3DModel(const std::string &filename, bool swapyz = true, float dx = 0, float dy = 0, float dz = 0, float markerLength = 0.1f, float scaleFactor = 1.0f)
    {
        this->dx = dx;
        this->dy = dy;
        this->dz = dz;
    ...
    void load(const std::string &filename, bool swapyz)
    {
        ...
        while (std::getline(file, line))
        {
            ...
            else if (type == "vt")
            {
                ...
                {
                    throw std::runtime_error("Error parsing texture coordinate data");
                }
