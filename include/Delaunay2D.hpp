...
namespace delaunay2d
{
  ...
  {
    T x, y;
    size_t index; // Añadido para almacenar el índice del punto

    Point() : x{0}, y{0}, index{0} {}
    Point(T _x, T _y, size_t _index) : x{_x}, y{_y}, index{_index} {}

    template <typename U>
    Point(U _x, U _y, size_t _index) : x{static_cast<T>(_x)}, y{static_cast<T>(_y)}, index{_index}
    {
    }

    friend std::ostream &operator<<(std::ostream &os, const Point<T> &p)
    {
      os << "x=" << p.x << "  y=" << p.y << "  index=" << p.index;
      return os;
    }
  ...
  Delaunay<T> triangulate(const std::vector<Point<T>> &points)
  {
    ...
    const auto p0 = Node{midx - 20 * dmax, midy - dmax, std::numeric_limits<size_t>::max()};
    const auto p1 = Node{midx, midy + 20 * dmax, std::numeric_limits<size_t>::max()};
    const auto p2 = Node{midx + 20 * dmax, midy - dmax, std::numeric_limits<size_t>::max()};
    ...
    for (size_t i = 0; i < points.size(); ++i)
    {
      ...
      {
        tmps.push_back({e.p0, e.p1, {pt.x, pt.y, i}});
      }
      d.triangles = tmps;
    }
    ...
    d.triangles.erase(
        std::remove_if(d.triangles.begin(), d.triangles.end(),
                       [&](auto const &tri)
                       {
                         return ((tri.p0.index == std::numeric_limits<size_t>::max()) ||
                                 (tri.p1.index == std::numeric_limits<size_t>::max()) ||
                                 (tri.p2.index == std::numeric_limits<size_t>::max()));
                       }),
        d.triangles.end());
